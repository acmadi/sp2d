	...
	<div data-options="region:'center'">
		...
		<div data-options="region:'south',split:true " style="height:200px;" > 
			<div class="easyui-tabs" id="tabX"  data-options="fit:true">
				<div title="Form Tambah Tahun" id="formBottom"  style="padding:3px 0px 0px 10px; "  data-options="">
						<form id="F-INPUT"  method="post" action="{{route('tahun.store')}}">
							...
							<div id="ForInput" style="width:70%; display:inline-block; top:0px; float:left; ">
								<div style="padding: 2px 0px 10px 10px;">
									...
									<label for="tahun" style="display:inline-block; width: 20%">Tahun </label>

									: <input  type="text" name="tahun" class="easyui-validatebox textbox"   placeholder="tahun" data-options="prompt:'Nama Tahun ', required:true "/>

								</div>
								<div style="padding: 2px 0px 10px 10px;">
									<label for="tahun" style="display:inline-block; width: 20%">jadikan Default </label>
									: <input  type="text" name="default" class="easyui-validatebox textbox"   placeholder="default " data-options="prompt:'Pilih default', required:true "/>
								</div>
					<!-- 			<div style="padding: 2px 0px 10px 10px;"> 
									<label style="display:inline-block; width: 20%"for="nama_singkat">Nama Singkatan  </label>

									: <input type="text" name="nama_singkat_Tahun" 
									class="easyui-validatebox textbox"   placeholder="Nama Singkat Tahun" data-options="prompt:'Nama Singkat Tahun', required:true " />
...
function FormTambah (argument) {
	$.get('{{route('token')}}', function(data) {
		...
		customTitleTab('tabX','Tambah Data Tahun Baru')
...
function AksiKoreksi (argument) {
	...
	if (row){
		 ...
		 $('#update').show();
	 	$('#simpan').hide();
	 	customTitleTab('tabX','Koreksi Data Tahun '+row.tahun)
	 	// var pp = $('#tabX').tabs('getSelected'); 
	 	// // var tab = pp.panel('options').tab;
	 	//  $('#tabX').tabs('update',{
		 // 	tab:pp,
		 // 	options : {
		 // 	title : 'Koreksi Data " '+row.tahun+' "'
		 // 	}
	 	// }); 
	
		 // $('#F-INPUT').form('load',row);
		 $('#F-INPUT').form('load','{{url('tahun')}}/'+row.id+'/edit');
		 $('#dds').layout('expand','south');
		 if ($('#F-INPUT').find("input[name='_method']").length <= 1) {
...
function AksiHapus (argument) {
	...
	if (row){
	 	console.log(row.id);
	 	$.messager.confirm('Konfirmasi ', 'Apakah Anda ingin menghapus data '+row.tahun, function(r){
			if (r)
			{
				 	$.post('{{url('tahun')}}/'+row.id, { _method: 'DELETE' }, function(data, textStatus, xhr) {
				 		// console.log(data)
...
/* setting loading page=================================================================*/
	...
	// title:'Daftar Detail Tahun',
	toolbar: '#tb',
		columns:[[
		// {field:'productid',title:'No',width:200},
		{field:'tahun',title:'Nama Tahun ',width:20}
		]],
		// height: 200,
		pagination : true,
		// remoteSort:true,
	...
	$(function(){
	    ...
	    $('#simpan').bind('click', function(){
	        	...
	        	$('#F-INPUT').form('submit',{  url:'{{route('tahun.store')}}',
	        							...
	        							success: function(result){
	        								if (result.code ==  200 ){
	        									$.messager.show({  
	        										title: 'Status',  
	        										msg: 'Data Tahun Berhasil Dimasukkan !'  
	        									});
	        									$('#F-INPUT').form('clear');
	        									// $('#this').dialog('close')
	        									$('#contentCenter').datagrid('reload');
	    ...
	    $('#update').bind('click', function(){
	        ...
	        // console.log($('#id').val());return false;
	        	$('#F-INPUT').form('submit',{ url:'{{url('tahun')}}/'+$('#id').val(), 
	        							onSubmit: function(param){
            								    param._method = 'PATCH';
            								    var isValid = $(this).form('validate');
            								    if (!isValid){
