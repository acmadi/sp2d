...
class DokumensController extends Controller
{
    ...
    /**
     ****************************PENAMBAHAN MULTIPLE FILE ******************************************************************* 
     **************************************SEHINGGA NAMA DI urutan *********************************************************
     */
   private function simpanFileToDirektori($file,   $dokumen_id,   $req,   $urutan)
   {
            // dd($req->all());
      if ($file->getClientMimeType()=='application/pdf') {
             ...
             $lokasi_tujuan= $req->get('no_boks');
                 ...
                 $entry = File::where('dokumen_id','=',$dokumen_id)->where('urutan','=',$urutan)->first();
                 // dd($entry );
                 // $update_object= File::find($entry->id);
// format nama file = ==============================================================================================
                $nama_file= str_slug( str_replace('/', '-', $req->get('no_sppd'))).'-'.str_slug( str_replace('/', '-', $req->get('nama_dokumen'))).'-'.$urutan.'-'.$file->getFilename().'.'.$extension;
                // echo $nama_file;
                // dd($entry);
                 ...
                 // dd(count($entry->get()->toArray()));
                    ...
                    $update= [
                                    ...
                                    // $update_object->mime =  $file->getClientMimeType();
                                    // $update_object->original_filename =  $file->getClientOriginalName();
                                    // $update_object->filename = $nama_file;
                                    // $update_object->nama_baru = $nama_file;
                                    // $update_object->dir = $lokasi_tujuan;
                                    // $update_object->urutan = $urutan;
                     if ($entry->update($update) ) {
                     // if ($update_object->update($update) ) {
                     // if ($update_object->save() ) {
    ...
    public function anyData(Request $req)
    {
        ...
        // $data = new Dokumens;
        // $data = \DB::table('dokumen')->select('id','no_sppd');
        // default count 
        // $count=$data->get()->count();
        $count=with(new Dokumens)->get()->count();
            ...
            foreach ($this->caris as $key =>$cari) {
                    ...
                    if ($req->get('sort') && $req->get('order')) {
                        ...
                        $data->orderBy('updated_at');
                         // $data->orderBy('no_sppd','ASC');
                    }
                   
                /*count rows ==========================================================================================*/
                // $count=$data->orderBy('no_sppd','ASC')->get()->count();
                $count=$data->get()->count();
            /* paging requestt==================================================================================================*/
            // if ($req->get('page')) {
            //     // dd($req->get('page')-1);
     ...
     public function show_relasi_kolom($collection='')
    {
        return $collection->each(function ($item, $key) {
            ...
            // var_dump($item->penerima);
             ...
             $item['file_status']=!empty($item['file'][0]->original_filename)?$item['file'][0]->original_filename:'';
             $item['email']=!empty($item['user']->email)?$item['user']->email:'--';
             // dd($item['file']->created_at);
             $item['tgl_input_file']=!empty($item->created_at)?$item->created_at->format('d/m/Y h:i:s A'):'--';
             ...
             elseif($item['file'] !== null){
                ...
                // }
                    ...
                    // $url=route('file.get',['dir'=>'default','filename'=>$item['file']['filename']]);
                    $url=route('file.get',['id'=>base64_encode($item['file'][0]['id'])]);
     ...
     * @return Response
     */
    private function daftar_libur_nasional()
    {   
        // $liburs = Tahun::where('default',Session::all('tahun'))->liburs;
        // $liburs = Tahun::with('liburs')->where('default',Session::get('tahun'));
        // $liburs = Tahun::where('default',Session::get('tahun'))->first()->liburs;
        $liburs = Tahun::find(Session::get('tahun'))->liburs;
        
        // dd($liburs->get());
        // dd($liburs->first()->liburs);
        return $liburs;
    }
   public function create()
   {
        // dd(Session::get('tahun'));
        // dd(Session::all('tahun'));
    ...
    {   
        ...
        // dd($req->all());
         ...
         $fileS = $req->file('upload-images')?$req->file('upload-images'):$req->file('gambar');
        $lokasi_tujuan= $req->get('no_boks');
        // $lokasi_tujuan=str_slug( str_replace('-', ' ', $req->get('no_boks')));
         if ($fileS) {
                $result;
                // dd($fileS);
                foreach ($req->file('upload-images') as $urutan => $file) {
                        $result = $this->simpanFileToDirektori($file,$id,$req,$urutan);
                }
                // dd($result);
                return $result;
